...
// Signature "flux" palette — vivid purple/magenta to match portfolio theme by default
const FLUX_FROM = "var(--flux-from, #e945f5)";
...
// and a deep inset for depth.
const BAR_SHADOW = `0 0 18px color-mix(in oklab, ${FLUX_FROM} 55%, transparent), 0 0 32px color-mix(in oklab, ${FLUX_TO} 40%, transparent), inset 0 1.5px 0 rgba(255, 255, 255, 0.5), inset 0 -2px 3px rgba(0, 0, 0, 0.45)`;
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center text-3xl font-semibold tracking-tight text-neutral-200 sm:text-4xl",
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative h-4 sm:h-5 w-full overflow-hidden rounded-full bg-white/10 shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)] border border-white/10",
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}

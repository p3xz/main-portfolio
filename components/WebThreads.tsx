...
const WebThreads: React.FC<WebThreadsProps> = ({
  ...
  const mouseRef = useRef<{ enabled: boolean; strength: number }>({ enabled: true, strength: 0.25 });
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSpeed: { value: prefersReducedMotion ? 0.02 : speed },
        ...
        uEnableMouse: { value: prefersReducedMotion ? 0.0 : 1.0 },
    ...
    const onMouseEnter = () => {
      if (!prefersReducedMotion) targetActive = 1;
    };
    const onMouseLeave = () => {
      targetActive = 0;
    };
    ...
    const loop = (t: number) => {
      ...
      (program.uniforms.uEnableMouse as { value: number }).value = (mouseRef.current.enabled && !prefersReducedMotion) ? 1.0 : 0.0;
